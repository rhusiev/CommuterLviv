/// Everything arriving over the socket. Widgets listen to [changes] for the
/// rare things; the painter reads [vehicles] directly every tick, so moving
/// vehicles cost a repaint rather than a rebuild.
library;
...
/// Eased between positions, which arrive every five seconds.
...
/// Interpolated position. Heading takes the short way round, so turning past
/// north does not spin the long way.
...
class Live extends ChangeNotifier {
  ...
  /// A stopwatch, so a clock correction cannot jump mid-animation.
  ...
  void _frame(Uint8List buf) {
    ...
    // Moves alone change nothing a widget shows; the painter repaints anyway
