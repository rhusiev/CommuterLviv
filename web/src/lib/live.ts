...
/** Milliseconds a vehicle takes to slide to its new position. Positions arrive
...
/** Everything arriving over the socket. Positions are deliberately outside
...
export class Live {
  ...
  /** A stable object per meaningful change: `useSyncExternalStore` tears if a
   * new object is returned on every call. */
...
/** Where a vehicle is now, between the last two reports. Heading takes the
 * short way round, so turning past north does not spin the wrong way. */
