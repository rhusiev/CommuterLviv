...
/** One pre-rendered badge per route, blitted rather than `fillText`ed per frame.
 * Rebuilt only when the device pixel ratio changes. */
...
export function colour(short: string, type: string): string {
  // A band per kind, and a hue within it per route
  const base = type === "tram" ? 200 : type === "trolleybus" ? 260 : 20;
  return `hsl(${(base + (hue(short) % 90)) % 360} 70% 52%)`;
}

/** The number without the kind letter the city puts in front of it: `А25` is a
 * bus, `Т07` a tram, `Тр33` a trolleybus. */
