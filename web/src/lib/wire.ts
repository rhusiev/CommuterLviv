/** The reader for the binary map frames. Mirrors `commuterlviv/live/wire.py`, and
 * the constants must stay identical to it: a frame carries no box of its own.
 *
 * Ten bytes per vehicle, read straight out of the DataView into flat arrays.
 * Nothing here allocates per vehicle - four hundred small objects per second is
 * the one thing that would show up in a frame budget. */
 ...
 * the vehicle is under way and its direction is worth drawing. Without this the
 * heading is still the route's direction at that point, but it says where the
 * vehicle will go rather than where it is going. */
