...
Everything that touches the tracker or the model runs in one worker thread, one
call at a time, behind a lock. That is not for safety against other threads -
there is only ever one - it is to keep about 200 ms of numpy per epoch off the
event loop, where 200 ms is twelve dropped frames on every open map.

The service does not record. The collector already runs beside it and is the
thing writing `feed.db`; giving the recorder a second author while it is part
way through a multi-day dataset is the one change here that could lose data.
The two poll the same endpoint independently, which costs one extra request
every five seconds and no correctness.
"""
import asyncio
import sqlite3
import time
...
class Service:
    ...
    def warm(self, db=None):
        """Prime the model from the recording, when there is a fresh one.

        A cold model knows only the timetable, so the first twenty minutes of a
        restart are the worst predictions the service ever makes. Replaying the
        last two hours of what the collector has already written fixes that in
        about twenty seconds, because a replay runs some three hundred times
        faster than the city does.
        """
        db = db or replay.DB
        try:
    ...
    async def epoch_loop(self):
        """On the absolute 60 s grid, so the live epochs land on the same
        boundaries the offline replay uses and the two can be compared."""
        while True:
            step = self.set.epoch
