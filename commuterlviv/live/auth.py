...
Sessions are opaque and server-side: the cookie is 32 random bytes and nothing
is signed into it, so revoking one is a DELETE.

Remember-me is the series-and-token scheme with constant-time comparisons; a
spent token coming back is treated as theft and drops the series and every
...
async def register(pool, ph, username, password, code=None, mode="code"):
    """Create an account. Returns (user_id, None) or (None, why not). The code
    is spent in the same transaction as the insert and the name clash comes
    from the unique index, so neither can be raced."""
...
async def login(pool, ph, username, password):
    """The user, or None; an unknown username still pays for one argon2 verify
    so the response time does not say which accounts are real."""
    name = security.clean_username(username)
...
async def load_session(pool, raw, idle, max_life, touch=True):
    """The session behind a cookie, or None. Both expiries are enforced here,
    never by the cookie; idle expiry is refreshed at most once a minute, and
    not at all when `touch` is false."""
    if not raw:
        return None
    row = await pool.fetchrow(
...
async def consume_remember(pool, raw, days, ip=None):
    ...
    Returns (user_id, new cookie), (None, None) when unknown or expired, or
    (None, "theft") when a spent token comes back."""
    ...
    async with pool.acquire() as con:
        async with con.transaction():
            ...
            if security.same(row["token_hash"], presented):
                ...
                # two tabs woke together; the loser of the race is not a thief
...
async def sweep(pool):
    """Delete what has expired; every read already filters on expiry anyway."""
    async with pool.acquire() as con:
