...
A vehicle is ten bytes; positions are 16-bit offsets on a fixed box around the
city, about half a metre per step. The box is a constant, not derived from the
data, so a client decodes a frame with no state and two frames encoded a minute
apart mean the same thing.

A snapshot carries every vehicle; a delta carries only the changed rows plus the
ids that are gone. Both share a header.
"""
import struct
...
def encode(veh, t, kind=SNAPSHOT, gone=()):
    ...
    Vehicles outside the box are dropped, not clipped onto its edge."""
    veh = inbox(veh)
...
def changed(prev, cur):
    """Which rows a client holding `prev` still needs, and which ids to drop.

    Compared on the encoded values, not the floats behind them, so a fix that
    moved less than one step counts as unchanged.
    """
    if not len(prev):
