...
Position is one number - distance along the trip's shape - from a two-state
Kalman filter on (distance, speed). The between-fix increment comes from the
vehicle's odometer, not from differencing map-matched positions, whose error is
correlated between neighbouring fixes; GPS only corrects absolute position.

Time is reported split into rolling and standing, because standing time does not
scale with distance and belongs to the place it happened.
"""
import numpy as np
...
class Cell:
    ...
    Reported while in progress, so slow crossings are not heard about last.
    Pace and hold are totals so far; only the weights are incremental, so one
    crossing contributes its own weight and no more.
    """
    ...
    def take(self, expected, final=False):
        ...
        A cell joined half way through is never reported: its totals are partial.
        """
...
def _spread(tr, s0, dist, gap, holding):
    """Charge this interval's time to the cells it happened in: standing time
    whole to one cell, rolling time split in proportion to distance."""
    cl = tr.clen
