"""The model's approaches as switches: `full` ships, each variant differs from
it in one place. Run with `python3 -m commuterlviv experiment`."""
...
class Config:
    ...
    # k_*: observations a layer needs before it outweighs the layer it backs off
    # to. *_hl: half-lives, in seconds; 0.0 disables the term. `prof_hl` is not a
    # term but a back-off target: one number per unit per hour of the day.
    k_unit: float = 4.0
    k_corr: float = 4.0
    fast_hl: float = 480.0
