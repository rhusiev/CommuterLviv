"""Continuously record the three Lviv feeds into SQLite, append-only.
...
Built to survive days of running: failing feeds back off, failed writes are
retried then dropped, the queue is bounded, and the run stops itself while there
is still disk left. A heartbeat line says which of those is happening.
"""
import argparse
import collections
import datetime
...
def emit(kind, row):
    """Queue a row, dropping rather than blocking: a stalled writer must not
    stall collection."""
    try:
...
def writer():
    ...
    def flush():
        """Commit what is pending; a batch that fails twice is dropped."""
        nonlocal db
        for attempt in (0, 1):
            try:
                if db is None:
...
def poll_loop(feed, period, once):
    """Run `once` forever; each consecutive failure doubles the wait, one
    success clears it."""
    fails = 0
...
def trips(deadband):
    """One row per prediction that moved further than `deadband`.

    The feed republishes every stop of every trip on every poll, mostly jitter.
    """
    last = {}
...
def collect_lad(period, limit):
    """The per-stop arrivals board, one stop at a time, spread over the period."""
...
def prune(keep_days):
    ...
    Shrinks the file only if the database was created with auto_vacuum on.
    """
...
def heartbeat(min_free_gb, keep_days):
    """Health line every HEARTBEAT_S, and the only thing that stops the run
